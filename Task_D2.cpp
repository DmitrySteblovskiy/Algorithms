...
// А вы сможете? Да.
// https://contest.yandex.ru/contest/20642/run-report/45735856/
...
bool LessPolarAngle(const Vector2D &v1, const Vector2D &v2) {     // сортировка
    bool less_angle = (v1.x * v2.y - v1.y * v2.x > 0);
    return less_angle;
}
...
std::vector<Vector2D> MinkowskiSum(const std::vector<Vector2D> &polygon_1, const std::vector<Vector2D> &polygon_2) {      // поиск суммы Минковского с использованием сортировки
    std::vector<Vector2D> polygon1 = polygon_1;
    std::vector<Vector2D> polygon2 = polygon_2;
...
long double Gauss(const std::vector<Vector2D> &polygon) {      // с помощью формулы Гаусса считаем площади выпуклых многоугольников, используя лишь координаты их вершин
